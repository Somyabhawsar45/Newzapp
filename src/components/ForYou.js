...
export default function ForYou() {
  ...
  const fetchRecs = useCallback(async () => {
    ...
    try {
      ...
      if (recData.reason === 'not_enough_history') { setStatus('no_history'); return; }
  ...
  return (
    <>
      <style>{`
        body {
          --fy-card-bg: #ffffff;
          --fy-card-border: #e2e8f0;
          --fy-card-hover-border: #6366f1;
          --fy-heading: #1a1a2e;
          --fy-hr: #e2e8f0;
        }
        body.dark-mode {
          --fy-card-bg: #1a1a2e;
          --fy-card-border: #2a2a4a;
          --fy-card-hover-border: #6366f1;
          --fy-heading: #ffffff;
          --fy-hr: #2a2a4a;
        }
  ...
  display: none;
      ...
      <div className="container" style={{ marginTop: '80px', marginBottom: '-80px' }}>
        ...
        <div className="fy-scroll" style={{ display: 'flex', gap: '16px', overflowX: 'auto', paddingBottom: '8px', scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          {recs.map((article, i) => (
            <a
              ...
              onMouseEnter={e => {
                e.currentTarget.style.borderColor = 'var(--fy-card-hover-border)';
                e.currentTarget.style.transform = 'translateY(-2px)';
              }}
              onMouseLeave={e => {
                e.currentTarget.style.borderColor = 'var(--fy-card-border)';
        ...
        <hr style={{ borderColor: 'var(--fy-hr)', margin: '24px 0 0' }} />
      </div>
    </>
  );
}